...
BRIDGE_SENTENCES: Dict[Tuple[str, str], List[str]] = {
    ...
    ('coding', 'rights'): [
        ...
        # --- GDPR, Privacy Engineering e Compliance ---
        "Implementa un modulo Python che verifica automaticamente se un form di raccolta dati rispetta i requisiti GDPR per il consenso esplicito.",
        "Scrivi il codice per un sistema di data masking che offusca automaticamente i campi sensibili (CF, IBAN, email) in un database PostgreSQL.",
        "Sviluppa uno script che scansiona un codebase Python alla ricerca di variabili che potrebbero contenere PII non cifrate e genera un report.",
        "Scrivi il codice per implementare la data retention automatica: cancellazione programmata dei record scaduti secondo la policy GDPR aziendale.",
        "Implementa in Python un registro automatico dei trattamenti (Art. 30 GDPR) che tracci operazioni CRUD su tabelle contenenti dati personali.",
        "Codice per un middleware Django che inietta automaticamente gli header di sicurezza HTTP richiesti dalle normative (HSTS, CSP, X-Frame-Options).",
        "Scrivi uno script che genera report di conformità GDPR analizzando i log di accesso al database e identificando accessi non autorizzati.",
        "Implementa un sistema di gestione delle richieste DSAR (Data Subject Access Request) che estrae ed esporta tutti i dati di un utente in formato JSON.",
        "Scrivi il codice per anonimizzare irreversibilmente un dataset prima della pubblicazione, rispettando i criteri di k-anonimity e l-diversity.",
        "Sviluppa un'API REST Flask che implementa il consenso granulare per diversi scopi di trattamento secondo la direttiva ePrivacy.",
        # --- Sicurezza Informatica e Diritto Penale ---
        "Scrivi il codice per un sistema di intrusion detection basato su regole che genera alert legalmente documentabili per l'art. 615-ter c.p.",
        "Implementa in Python un sistema di logging immutabile con hash chain per garantire la non ripudiabilità delle prove digitali in giudizio.",
        "Scrivi il codice per la verifica dell'integrità di file digitali tramite hash SHA-256 con timestamp certificato per validità probatoria legale.",
        "Sviluppa uno script che implementa un canale di segnalazione sicuro e anonimo per il whistleblowing conforme al D.Lgs. 24/2023.",
        "Codice per un sistema di autenticazione multi-fattore (TOTP + biometrico) conforme alle linee guida AgID per i servizi della PA.",
        "Implementa in Python la gestione sicura delle chiavi crittografiche secondo gli standard FIPS 140-2 richiesti per i sistemi governativi.",
        "Scrivi il codice per un sistema di DLP (Data Loss Prevention) che monitora e blocca l'esfiltrazione di dati classificati dalla rete aziendale.",
        "Sviluppa uno script per il penetration testing passivo che verifica la conformità TLS/SSL rispetto alle normative PCI-DSS.",
        # --- Proprietà Intellettuale e Software ---
        "Scrivi il codice per uno scanner che analizza le dipendenze di un progetto Python e verifica la compatibilità delle licenze open source (GPL, MIT, Apache).",
        "Implementa uno script che confronta due codebase Python usando hashing e AST per rilevare potenziali violazioni del diritto d'autore sul codice.",
        "Codice per un sistema automatico di watermarking steganografico del codice sorgente per tracciarne la distribuzione non autorizzata.",
        "Scrivi il codice per un'API che gestisce automaticamente le royalties di contenuti digitali eseguendo micro-pagamenti via smart contract Solidity.",
        "Sviluppa uno script che analizza i commit Git per identificare contributi di autori esterni e verificare la catena di titolarità del codice.",
        # --- Contratti Digitali e Smart Contract ---
        "Implementa uno smart contract Solidity per la gestione automatica di un contratto di locazione con pagamento escrow e penali automatiche.",
        "Scrivi il codice per un sistema di firma digitale qualificata (QES) conforme al Regolamento eIDAS per documenti contrattuali vincolanti.",
        "Sviluppa in Python un parser di contratti in formato XML che estrae automaticamente le clausole penali e le scadenze per un sistema di alert.",
        "Codice per uno smart contract che implementa una DAA (Distributed Autonomous Agreement) con risoluzione automatica delle controversie.",
        "Scrivi il codice per un sistema di timestamping legalmente valido su blockchain per la data certa dei documenti (art. 2704 c.c.).",
        # --- Diritto del Lavoro e Automazione HR ---
        "Scrivi il codice Python per un sistema HR che verifica automaticamente la conformità dei contratti di lavoro rispetto al CCNL di riferimento.",
        "Implementa uno script che calcola automaticamente TFR, ferie maturate e straordinari rispettando i vincoli normativi del D.Lgs. 66/2003.",
        "Codice per un sistema di monitoraggio del lavoro agile che rispetta i limiti legali sulla disconnessione (D.Lgs. 81/2017 e L. 81/2017).",
        "Sviluppa un'API che genera automaticamente le buste paga verificando la conformità ai minimi tabellari e alle detrazioni IRPEF vigenti.",
        "Scrivi il codice per un sistema di anonimizzazione dei dati di performance dei dipendenti conforme allo Statuto dei Lavoratori art. 4.",
        # --- Amministrazione Digitale e PA ---
        "Implementa in Python il parsing e la validazione di fatture elettroniche XML nel formato FatturaPA richiesto dall'Agenzia delle Entrate.",
        "Scrivi il codice per integrare il sistema di autenticazione SPID/CIE tramite SAML2 in un'applicazione web Flask seguendo le specifiche AgID.",
        "Codice per la generazione automatica di notifiche PEC (Posta Elettronica Certificata) con validità legale tramite API Python.",
        "Sviluppa uno script che verifica la firma digitale di documenti PDF firmati con CAdES o PAdES secondo le specifiche CADe italiane.",
        "Implementa un sistema di conservazione a norma (D.Lgs. 82/2005 CAD) per documenti digitali con metadati e hash certificate.",
        # --- AI Act e Sistemi Algoritmici ---
        "Scrivi il codice per implementare un sistema di audit trail obbligatorio per sistemi AI ad alto rischio secondo l'Art. 12 dell'AI Act europeo.",
        "Implementa in Python un modulo di spiegabilità (LIME/SHAP) per rendere interpretabili le decisioni automatizzate ai sensi dell'Art. 22 GDPR.",
        "Codice per un sistema di monitoraggio della deriva (data drift) di un modello ML in produzione con alert per la revisione umana obbligatoria.",
        "Scrivi il codice per un registro automatico delle decisioni algoritmiche che garantisca la tracciabilità richiesta dal Regolamento AI Act per i sistemi ad alto rischio.",
        "Sviluppa un framework di test automatico per la valutazione del bias algoritmico in un modello di credit scoring conforme alle direttive EBA.",
        "Implementa uno script che verifica se un modello di ML soddisfa i criteri di fairness richiesti dalla normativa antidiscriminatoria europea (Dir. 2000/43/CE).",
    ],
    ...
    ('coding', 'math'): [
        ...
        # --- Algebra Lineare e Decomposizioni ---
        "Scrivi il codice Python per la decomposizione LU di una matrice densa e usa il risultato per risolvere Ax=b.",
        "Implementa in NumPy la fattorizzazione di Cholesky e spiega perché è applicabile solo a matrici definite positive.",
        "Scrivi il codice per il calcolo della pseudoinversa di Moore-Penrose tramite SVD in Python.",
        "Implementa in Python il metodo del gradiente coniugato per risolvere sistemi lineari sparsi di grandi dimensioni.",
        "Scrivi il codice che calcola il numero di condizionamento di una matrice e spiega le sue implicazioni numeriche.",
        "Implementa l'algoritmo di Lanczos in Python per stimare gli autovalori estremi di una matrice sparsa simmetrica.",
        "Codice per verificare computazionalmente se una matrice è diagonalizzabile confrontando molteplicità algebrica e geometrica.",
        "Scrivi il codice per calcolare l'esponenziale matriciale tramite la decomposizione in autovalori in Python.",
        "Implementa il metodo di Gram-Schmidt modificato in C++ per una maggiore stabilità numerica rispetto alla versione classica.",
        "Scrivi uno script Python che calcola la distanza di Frobenius tra due matrici e verifica la proprietà triangolare.",
        # --- Ottimizzazione Numerica ---
        "Implementa l'algoritmo di discesa del gradiente stocastico (SGD) da zero in Python e mostrarne la convergenza su una funzione convessa.",
        "Scrivi il codice per l'ottimizzatore Adam da zero e spiega la correzione del bias per i momenti del primo e secondo ordine.",
        "Implementa in Python il metodo di Newton per l'ottimizzazione di funzioni multivariabile usando il gradiente e la matrice Hessiana.",
        "Codice per il metodo BFGS quasi-Newton in Python con aggiornamento dell'approssimazione dell'inversa dell'Hessiana.",
        "Implementa il metodo del punto fisso in Python per trovare soluzioni di equazioni non lineari e analizza la velocità di convergenza.",
        "Scrivi il codice per l'algoritmo di ottimizzazione PSO (Particle Swarm Optimization) e testa su una funzione non convessa.",
        "Implementa il metodo di backtracking line search in Python per garantire la discesa sufficiente nell'ottimizzazione.",
        "Scrivi il codice per risolvere un problema di programmazione quadratica con vincoli di uguaglianza usando i moltiplicatori di Lagrange.",
        # --- Probabilità, Statistica e Analisi Dati ---
        "Implementa da zero in Python il calcolo dell'intervallo di confidenza bootstrap per una statistica arbitraria.",
        "Scrivi il codice per il test di Kolmogorov-Smirnov e spiega la statistica del test e la distribuzione del p-value.",
        "Implementa l'algoritmo EM (Expectation-Maximization) in Python per stimare i parametri di una miscela di gaussiane.",
        "Codice per calcolare la correlazione di Pearson e Spearman su due serie temporali e testarne la significatività statistica.",
        "Scrivi il codice per simulare una catena di Markov a stati finiti e calcolare la distribuzione stazionaria empiricamente.",
        "Implementa in Python la stima della densità kernel (KDE) con bandwidth ottimale scelto tramite validazione incrociata.",
        "Scrivi il codice per un test chi-quadrato di adattamento e spiega come si calcola il grado di libertà corretto.",
        "Implementa il filtro di Kalman in Python per la stima dello stato di un sistema dinamico lineare con rumore gaussiano.",
        "Codice per calcolare l'entropia di Shannon e l'informazione mutua tra due variabili aleatorie discrete da dati campionari.",
        # --- Teoria dei Grafi e Combinatoria ---
        "Implementa l'algoritmo di Floyd-Warshall in Python per i cammini minimi tra tutte le coppie di nodi e analizza la complessità O(V³).",
        "Scrivi il codice per calcolare il polinomio caratteristico di una matrice di adiacenza e trovarne gli autovalori spettrali.",
        "Implementa in Python l'algoritmo di Ford-Fulkerson per il flusso massimo e verifica il teorema max-flow min-cut.",
        "Codice per generare e analizzare grafi casuali di Erdős–Rényi e calcolare la probabilità di connessione in funzione di p.",
        "Scrivi il codice per trovare i componenti fortemente connessi in un grafo diretto usando l'algoritmo di Tarjan con complessità O(V+E).",
        "Implementa l'algoritmo di Bellman-Ford in Python e verifica la rilevazione di cicli negativi nel grafo pesato.",
        # --- Equazioni Differenziali e Simulazione ---
        "Implementa in Python il metodo di Eulero esplicito e implicito per risolvere una EDO rigida e confronta la stabilità numerica.",
        "Scrivi il codice per la simulazione di un sistema dinamico caotico (Lorenz) usando Runge-Kutta 4 e visualizza l'attrattore.",
        "Implementa il metodo degli elementi finiti monodimensionale in Python per risolvere equazioni differenziali al contorno.",
        "Codice per risolvere numericamente la trasformata Z inversa tramite il metodo dei residui implementato in Python.",
        "Scrivi il codice per simulare la diffusione di calore in una barra unidimensionale con differenze finite e condizioni al contorno di Dirichlet.",
        "Implementa in Python il metodo di Crank-Nicolson per risolvere l'equazione di diffusione con maggiore stabilità rispetto a Eulero.",
        # --- Teoria dell'Informazione e Compressione ---
        "Scrivi il codice Python per l'algoritmo di Huffman e calcola il guadagno di compressione rispetto all'entropia di Shannon teorica.",
        "Implementa l'algoritmo aritmetico di codifica in Python e confronta l'efficienza con la codifica di Huffman su testi reali.",
        "Codice per calcolare la capacità del canale di Shannon per un canale binario simmetrico (BSC) al variare della probabilità di errore.",
        "Scrivi il codice per implementare la codifica turbo e calcolare il BER (Bit Error Rate) in funzione del SNR tramite simulazione.",
        # --- Geometria Computazionale ---
        "Implementa l'algoritmo di convex hull (Graham scan o Jarvis march) in Python e analizza la complessità computazionale.",
        "Scrivi il codice per triangolare un poligono semplice usando l'algoritmo ear-clipping e verifica la formula di Eulero.",
        "Codice per calcolare l'integrale di una funzione su un dominio irregolare usando il metodo di Monte Carlo geometrico.",
        "Implementa in Python l'interpolazione di Lagrange e confronta con gli spline cubici naturali su un insieme di punti campione.",
    ],
    ...
    ('math', 'rights'): [
        ...
        # --- Calcoli Fiscali e Tributari ---
        "Qual è la formula matematica per il calcolo dell'IRPEF lorda applicando le aliquote progressive per scaglioni previste dal TUIR?",
        "Come si calcola matematicamente l'imposta sostitutiva sulla rivalutazione del TFR e quali sono i coefficienti normativi vigenti?",
        "Dimostra il modello matematico per il calcolo dell'acconto IRES con il metodo previsionale e storico secondo la normativa tributaria.",
        "Qual è la formula normativa esatta per calcolare le sanzioni e gli interessi di mora sull'IVA non versata nei termini previsti?",
        "Come si determina matematicamente la base imponibile IMU per immobili di categoria catastale diversa secondo le aliquote comunali?",
        "Deriva la formula per il calcolo del credito d'imposta R&D e quali requisiti normativi deve soddisfare la spesa per essere agevolabile?",
        "Qual è il modello matematico per la determinazione del reddito d'impresa secondo il principio di competenza fiscale ex art. 109 TUIR?",
        "Come si calcolano matematicamente le quote di ammortamento fiscalmente deducibili per i beni strumentali secondo i coefficienti ministeriali?",
        # --- Finanza, Assicurazioni e Diritto ---
        "Qual è la formula attuariale per il calcolo del valore di riscatto di una polizza vita e come è regolata dall'IVASS?",
        "Come si determina matematicamente il premio assicurativo puro per una polizza RC Auto secondo i criteri tariffari IVASS?",
        "Dimostra la formula per il calcolo del VAR (Value at Risk) al 99 per cento e come viene applicata dai requisiti prudenziali di Basilea III.",
        "Qual è il modello matematico Black-Scholes per la valutazione delle opzioni e come regolamenta il Regolamento EMIR i derivati OTC?",
        "Come si calcola il coefficiente di Gini per misurare la disuguaglianza distributiva e come viene usato dalla normativa fiscale progressiva?",
        "Qual è la formula per il calcolo del tasso di sostituzione pensionistico nel metodo contributivo secondo la L. 335/1995 (Riforma Dini)?",
        "Dimostra come si calcola l'indice di Sharpe di un fondo d'investimento e quali obblighi di disclosure impone il Regolamento UCITS.",
        "Qual è il modello matematico per il calcolo del LEI (Livello Equivalente di Interferenza) e come si applicano i limiti normativi del D.Lgs. 81/2008?",
        # --- Diritto del Lavoro e Calcoli Retributivi ---
        "Qual è la formula matematica completa per calcolare l'indennità di licenziamento secondo il Jobs Act (D.Lgs. 23/2015) in funzione dell'anzianità?",
        "Come si calcola matematicamente il rateo di tredicesima maturato e quale normativa del CCNL ne disciplina la corresponsione?",
        "Dimostra la formula per il calcolo dell'assegno di integrazione salariale (ex CIG) e i vincoli normativi sulla durata massima.",
        "Qual è il modello matematico per la rivalutazione annuale del TFR con il coefficiente fisso e variabile secondo la L. 297/1982?",
        ...
        "Dimostra la formula per determinare il minimale contributivo INPS per lavoratori autonomi e le relative sanzioni per omissioni.",
        # --- Risarcimento del Danno e Calcoli Civilistici ---
        "Qual è il modello matematico per la capitalizzazione del danno biologico permanente secondo le tabelle del Tribunale di Roma e Milano?",
        "Come si calcola matematicamente il lucro cessante futuro attualizzato per un lavoratore infortunato secondo la giurisprudenza della Cassazione?",
        "Dimostra la formula attuariale per il calcolo della rendita vitalizia in sostituzione del danno da perdita del reddito ex art. 2056 c.c.",
        "Qual è il metodo matematico per la capitalizzazione dell'assegno di mantenimento in sede di divorzio secondo la giurisprudenza italiana?",
        "Come si determina quantitativamente la perdita di chance patrimoniale risarcibile e quali parametri probabilistici usa la Cassazione?",
        "Dimostra il calcolo degli interessi compensativi sull'obbligazione risarcitoria ex art. 1282 c.c. e come si coordinano con la rivalutazione ISTAT.",
        # --- Diritto Societario e Valutazioni d'Azienda ---
        "Qual è il metodo matematico DCF (Discounted Cash Flow) per la valutazione d'azienda e come si applica nel contesto della liquidazione giudiziale?",
        "Come si calcola matematicamente il valore di liquidazione delle quote societarie in caso di recesso del socio ex art. 2473 c.c.?",
        "Dimostra la formula per il calcolo della congruità del corrispettivo nelle fusioni per incorporazione ai fini della tutela dei soci di minoranza.",
        "Qual è il modello quantitativo per il calcolo dell'indennizzo in caso di espropriazione di partecipazioni secondo la normativa Golden Power?",
        "Come si calcola il test di impairment dell'avviamento (goodwill) secondo i principi IAS/IFRS e gli obblighi di disclosure nella nota integrativa?",
        # --- Diritto Bancario e Creditizio ---
        "Qual è la formula esatta per il calcolo del TEG (Tasso Effettivo Globale) secondo le istruzioni della Banca d'Italia per la verifica dell'usura?",
        "Dimostra matematicamente come si calcola l'interesse composto capitalizzato trimestralmente e quando configura anatocismo vietato ex art. 1283 c.c.",
        "Come si determina quantitativamente il danno da misselling di prodotti finanziari complessi secondo la Direttiva MiFID II?",
        "Qual è il modello matematico per il calcolo della soglia di usura trimestrale secondo la metodologia Banca d'Italia e la L. 108/1996?",
        ...
        # --- Statistiche Giuridiche e Prove ---
        "Come si calcola la probabilità a posteriori di una corrispondenza DNA e come il codice di procedura penale ne disciplina la valenza probatoria?",
        "Qual è il modello statistico per la determinazione del campione minimo in un'ispezione fiscale e come è regolato dallo Statuto del Contribuente?",
        "Dimostra come si calcola il tasso di mortalità infantile e come i parametri demografici vengono usati dalla normativa previdenziale INPS per le proiezioni.",
        "Come si determina matematicamente la significatività statistica di un comportamento discriminatorio nei dati HR ai fini del contenzioso giuslavoristico?",
