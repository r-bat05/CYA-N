...
Progetta l'architettura di un'app web per la gestione dei consensi cookie rispettando scrupolosamente la direttiva europea sulla privacy

Risolvi l'equazione differenziale lineare del secondo ordine a coefficienti costanti omogenea.

Spiegami in modo semplice come funziona la fotosintesi clorofilliana nelle piante a foglia larga.
...
Spiegami la differenza tra dolo, colpa cosciente e preterintenzione all'interno del codice penale italiano.

Scrivi una funzione in Python che ordina una lista di dizionari in base al valore di una chiave specifica.
...
Dammi una ricetta tradizionale per preparare la carbonara romana, con i dosaggi esatti.
...
Scrivi una funzione Rust che implementa un albero AVL con operazioni di inserimento e bilanciamento automatico. → [CODING]

Dimostra il teorema di Pitagora usando il metodo dei triangoli simili e l'approccio algebrico. → [MATH]

Quali sono i presupposti per il riconoscimento dello stato di insolvenza ai sensi della legge fallimentare? → [RIGHTS]

Come si prepara un risotto alla milanese con lo zafferano? → [GENERAL]

Scrivi uno script Bash che monitora in tempo reale l'utilizzo della CPU e invia un alert email se supera l'80%. → [CODING]

Dimostra che il problema dello zaino 0/1 è NP-completo tramite riduzione polinomiale e implementa una soluzione con programmazione dinamica. → [MATH→CODING]

Implementa in C++ un allocatore di memoria custom usando memory pool con blocchi a dimensione fissa. → [CODING]

Quali sono i criteri per la determinazione del foro competente in una controversia contrattuale internazionale (Reg. Bruxelles I bis)? → [RIGHTS]

Implementa un parser LR(1) in Python per analizzare espressioni aritmetiche con precedenza degli operatori. → [CODING]

Scrivi uno smart contract Solidity per la gestione di un testamento digitale che rispetti le quote di legittima previste dal codice civile italiano. → [CODING→RIGHTS]
...
Qual è il calcolo attuariale del valore attuale netto di una rendita perpetua crescente e quali norme del TUF regolano la sua commercializzazione? → [RIGHTS]
...
Spiega la crittografia omomorfica partendo dall'algebra degli anelli quoziente e implementa in Python un esempio di addizione su dati cifrati con il schema BGV semplificato. → [CODING]
...
Quali sono le responsabilità civili di un amministratore di sistema in caso di data breach secondo il GDPR e come si implementa un sistema di incident response automatizzato? → [RIGHTS]
