"""
    MOTORE AI IBRIDO V5.0 (Refactoring & Configuraizone Centralizzata)
    
    Novità:
    - Rimosse costanti hardcoded (ora legge da config.py).
    - Logica di gestione risorse (RAM) centralizzata e dinamica.
    - Gestione errori migliorata per Ollama e psutil.
"""
...
import config  # <-- Importa le configurazioni centralizzate

class BaseAI(ABC):
    """
    Classe astratta che gestisce la logica comune di generazione e controllo risorse.
    Ora si inizializza leggendo i parametri direttamente da config.MODELS_CONFIG.
    """
    def __init__(self, category):
        # Carica la configurazione specifica per la categoria (coding, math, rights...)
        if category not in config.MODELS_CONFIG:
            # Fallback di sicurezza su general se la categoria non esiste nel config
        ...
        # Mapping delle soglie RAM (da chiavi stringa a Byte reali)
        self.primary_ram_req = config.RAM_THRESHOLDS[self.cfg['ram_threshold']]
        
        # Se esiste un fallback, calcoliamo anche la sua RAM richiesta
    ...
    def check_resources(self):
        ...
        if self.is_using_fallback:
            if available_ram < self.fallback_ram_req:
                ...
                print(f"   Disponibili: {available_ram/config.GB:.2f} GB < Richiesti: {self.fallback_ram_req/config.GB:.2f} GB")
                return False 
            return True 
        ...
        if available_ram < self.primary_ram_req:
            ...
            print(f"   Disponibili: {available_ram/config.GB:.2f} GB < Richiesti: {self.primary_ram_req/config.GB:.2f} GB")
            
            if self.fallback_model:
                print(f"📉  Downgrade PREVENTIVO a [{self.fallback_model}]...")
                self.is_using_fallback = True
                # Ricorsione: controlliamo se abbiamo RAM per il fallback
    ...
    def generate(self, messages: list):
        ...
        # 3. Impostazioni di Sistema (Timeout e Context) da config.py
        options = {
        ...
        is_thinking = False # Flag per nascondere i tag <think>
        ...
        try:
            ...
            for chunk in stream:
                ...
                if "</think>" in content:
                    is_thinking = False
                    content = content.replace("</think>", "")
                    spinner.stop() # Interrompiamo lo spinner appena inizia la risposta vera
                
                if is_thinking:
                    continue 
                
                # Stampa output pulito
                if content:
                    display_content = clean_response(content) 
                    if display_content:
                        spinner.stop() # Sicurezza extra
            ...
            spinner.stop()
            return f"\n\n❌ Errore Generico: {e}\nVerifica la connessione o il modello ('ollama pull {target_model}')."
        ...
        # Reset del flag per la prossima chiamata
        self.is_using_fallback = False 
...
class CodeLlamaAI(BaseAI):
    ...
    def resolve(self, prompt: str):
        sys, few_shot, _ = get_prompts('coding')
        # Prompt Engineering specifico per il codice
        final_prompt = f"{few_shot}\n[RICHIESTA]: {prompt}\n\n[IMPORTANTE]: Spiega il codice e i concetti ESCLUSIVAMENTE IN ITALIANO."
...
class GptOssAI(BaseAI):
    def __init__(self, category='general'):
        # Questa classe gestisce sia 'rights' che 'general' in base al parametro
        super().__init__(category)

    def resolve(self, prompt: str):
        sys, few_shot, _ = get_prompts(self.category)
        full_user_content = f"{few_shot}\n[RICHIESTA UTENTE]: {prompt}\n\n[IMPORTANTE]: Rispondi IN ITALIANO."
        messages = [{'role': 'system', 'content': sys}, {'role': 'user', 'content': full_user_content}]
        return self.generate(messages)
...
def get_ai_model(category: str):
    """
    Factory che restituisce l'istanza corretta.
    Nota: La logica dei modelli è ora definita in config.py, 
    qui mappiamo solo la categoria alla classe Python che costruisce il prompt.
    """
    if category == 'coding': return CodeLlamaAI()
    elif category == 'math': return DeepSeekAI()
    elif category == 'rights': return GptOssAI(category='rights')
    else: return GptOssAI(category='general')